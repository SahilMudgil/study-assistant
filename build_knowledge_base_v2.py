"""
Study Assistant - Exercise 2 (Vision Model Version)
------------------------------------------------------
Course slides are often mostly diagrams/bullets rather than clean paragraphs,
so instead of extracting raw text, we:

  1. Render each PDF page as an image
  2. Ask a vision model (llava, via Ollama) to read/transcribe that slide
  3. Chunk the resulting text
  4. Embed each chunk (nomic-embed-text, via Ollama)
  5. Save everything to knowledge_base.json
...
def pdf_page_to_base64_image(page) -> str:
    """
    Renders a single PDF page as a PNG image and returns it as a base64 string,
    which is the format Ollama's vision models expect.
    """
    pix = page.get_pixmap(dpi=150)  # 150 dpi is a good balance of clarity vs size
...
def read_slide_with_vision_model(base64_image: str) -> str:
    """
    Sends a slide image to the vision model and asks it to transcribe
    everything on the slide (text, bullet points, and a description of
    any diagrams/charts).
    """
    payload = {
        "model": VISION_MODEL,
...
def chunk_text(text: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP):
    ...
    if len(words) <= chunk_size:
        return [text]  # short slide, no need to split further
...
def build_knowledge_base():
    ...
    for pdf_file in pdf_files:
        ...
        print(f"  {len(doc)} page(s) found. Reading each with the vision model...")
